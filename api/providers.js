...
module.exports = requireAuth(async (req, res) => {
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await supabase.from("api_providers").insert({
      name: String(name).trim(), base_url: String(base_url).trim().replace(/\/+$/, ""), api_key: String(api_key).trim(), active: true
    }).select().single();
    if (error) return res.status(500).json({ error: error.message });
  ...
  if (req.method === "PATCH") {
    const { id, name, base_url, api_key, active } = req.body || {};
    if (!id) return res.status(400).json({ error: "id مطلوب" });
    const patch = {};
    if (name !== undefined) patch.name = String(name).trim();
    if (base_url !== undefined) patch.base_url = String(base_url).trim().replace(/\/+$/, "");
